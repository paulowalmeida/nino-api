...
describe(InvoiceStatusController.name, () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: InvoiceStatusService, useValue: mockService }],
    }).compile()
  ...
  it('getAll() should return an array of invoice statuses', async () => {
    mockService.getAll.mockResolvedValue([mockInvoiceStatus])
  ...
  it('getById() should return an invoice status by id', async () => {
    mockService.getById.mockResolvedValue(mockInvoiceStatus)
  ...
  it('delete() should delete an invoice status', async () => {
    mockService.delete.mockResolvedValue({
      message: 'InvoiceStatus deleted successfully',
    })

    const result = await controller.delete('uuid-1')

    expect(result).toEqual({ message: 'InvoiceStatus deleted successfully' })
