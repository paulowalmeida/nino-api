...
describe('CredentialsService', () => {
  ...
  it('should create a credential with a hashed password if provided', async () => {
    ...
    expect(repository.create).toHaveBeenCalledWith({ userId: 'user-id', password: 'hashed-password' });
    expect(result).toEqual(mockCredential);
  });

  it('should create a credential without hashing if password is not provided', async () => {
    const result = await service.create({ userId: 'user-id', email: 'test@test.com' });
    expect(passwordService.hash).not.toHaveBeenCalled();
    expect(repository.create).toHaveBeenCalledWith({ userId: 'user-id', email: 'test@test.com' });
    expect(result).toEqual(mockCredential);
  });
  ...
  it('should update a credential hashing the password if provided', async () => {
    ...
    expect(passwordService.hash).toHaveBeenCalledWith('123');
    expect(repository.update).toHaveBeenCalledWith('cred-id', { password: 'hashed-password' });
  });
  ...
  it('should update a credential without hashing if password is not provided', async () => {
    ...
    expect(passwordService.hash).not.toHaveBeenCalled();
    expect(repository.update).toHaveBeenCalledWith('cred-id', { email: 'new@test.com' });
  });
