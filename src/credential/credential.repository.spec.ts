...
describe('CredentialsRepository', () => {
  ...
  it('should create a credential successfully', async () => {
    jest.spyOn(prismaService.authCredential, 'create').mockResolvedValue(mockCredential as any);
  ...
  it('should get a list of credentials by user id', async () => {
    jest.spyOn(prismaService.authCredential, 'findMany').mockResolvedValue([mockCredential] as any);
  ...
  it('should call handleError when getListByUserId throws an error', async () => {
    const error = new Error('db error');
    jest.spyOn(prismaService.authCredential, 'findMany').mockRejectedValue(error);
    await repository.getAll('user-id');
    expect(prismaErrorService.handleError).toHaveBeenCalledWith(error);
  });

  it('should get a credential by id', async () => {
    jest.spyOn(prismaService.authCredential, 'findUnique').mockResolvedValue(mockCredential as any);
  ...
  it('should throw NotFoundException and call handleError when getById finds nothing', async () => {
    jest.spyOn(prismaService.authCredential, 'findUnique').mockResolvedValue(null);
  ...
  it('should get a credential by email', async () => {
    jest.spyOn(prismaService.authCredential, 'findFirst').mockResolvedValue(mockCredential as any);
  ...
  it('should throw NotFoundException and call handleError when getByEmail finds nothing', async () => {
    jest.spyOn(prismaService.authCredential, 'findFirst').mockResolvedValue(null);
    await repository.getByEmail('test@test.com');
    expect(prismaErrorService.handleError).toHaveBeenCalledWith(expect.any(NotFoundException));
  });

  it('should update a credential successfully', async () => {
    jest.spyOn(prismaService.authCredential, 'update').mockResolvedValue(mockCredential as any);
  ...
  it('should delete a credential successfully', async () => {
    jest.spyOn(prismaService.authCredential, 'delete').mockResolvedValue(mockCredential as any);
