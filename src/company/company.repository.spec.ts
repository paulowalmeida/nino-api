...
describe('CompanyRepository', () => {
  ...
  it('getAll - deve retornar array de companies', async () => {
    ...
    expect(mockRepository.find).toHaveBeenCalledWith({ order: { companyName: 'ASC' } })
  })
  ...
  it('getByCnpj - deve retornar company por cnpj', async () => {
    ...
    expect(result).toEqual(mockCompany)
    expect(mockRepository.findOneBy).toHaveBeenCalledWith({ cnpj: '12345678000190' })
  })
  ...
  it('create - deve criar nova company', async () => {
    ...
    expect(result).toEqual(mockCompany)
    expect(mockRepository.findOneBy).toHaveBeenCalledWith({ cnpj: createData.cnpj })
  ...
  it('update - deve chamar errorService.handle com ConflictException se novo CNPJ já existe em outra empresa', async () => {
    const anotherCompany = { ...mockCompany, id: 'uuid-2', cnpj: '98765432000100' }
  ...
  it('activate - deve ativar company', async () => {
    ...
    mockRepository.findOneBy.mockResolvedValue({ ...mockCompany, isActive: false })
